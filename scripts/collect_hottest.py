...
def get_posts(sub_reddit_name):
    ...
    # TODO: Error Handling
    try:
        data = requests.get(f'http://api.reddit.com/r/{sub_reddit_name}/hot?limit={num_posts}',
...
def get_posts_after(sub_reddit_name, after_name):
    ...
    # TODO: Error Handling
    try:
        data = requests.get(f'http://api.reddit.com/r/{sub_reddit_name}/hot?limit={num_posts}&after={after}',
                            headers={'User-Agent': 'macos:requests (by /u/Sense_Sen_sibility)'})
    except requests.exceptions:
        print("Request Fails.")
        exit(0)
